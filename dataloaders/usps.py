...
def get(data_path,seed=0,pc_valid=0.10):
    ...
    path = os.path.join(data_path, 'binary_usps')
    if not os.path.isdir(path):
        ...
        # CIFAR10
        dat={}
        dat['train']=datasets.USPS(data_path,train=True,download=True,transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
        dat['test']=datasets.USPS(data_path,train=False,download=True,transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
        for n in range(5):
            data[n]={}
            data[n]['name']='usps'
            data[n]['ncla']=2
