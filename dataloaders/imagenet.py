...
def get(data_path,seed=0,pc_valid=0.10):
    ...
    if not os.path.isdir(path):
        ...
        # Imagenet
        dat={}
        dat['train']=datasets.ImageNet(data_path,split = train,download=True,transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
        dat['test']=datasets.ImageNet(data_path,split = test,download=True,transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
        for n in range(1):
            data[n]={}
